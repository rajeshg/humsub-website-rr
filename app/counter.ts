...
export class Counter extends DurableObject {
    ...
    this.stateObj.storage
      ...
      .then((order: unknown) => {
        ...
        if (Array.isArray(arr)) {
          ...
          for (const it of this.event.items) {
            if (it.type === 'PERFORMANCE' && (it as PerformanceItem).timer_start_time && (it as PerformanceItem).durationSeconds) {
              // schedule asynchronously
              try { this.scheduleTimerEnd(it as PerformanceItem); } catch {}
            }
          }
        }
      })
  ...
  private scheduleTimerEnd(item: PerformanceItem) {
    ...
    if (existing) {
      try { clearTimeout(existing as unknown as number); } catch {}
  ...
  return new Response(null, { status: 101, webSocket: client } as unknown as ResponseInit);
    ...
    const data = await request.json() as { itemId: string; state?: string; name?: string; timer_start_time?: number | null; timer_end_time?: number | null; type?: ItemType; durationSeconds?: number | null; description?: string | null; style?: string | null; teamSize?: number | null; choreographers?: string | null; duration?: string | null };
      const { itemId, state, name, timer_start_time, timer_end_time, type, durationSeconds, description, style, teamSize, choreographers, duration } = data;
      ...
      if (item) {
        ...
        if (item.type === 'PERFORMANCE') {
          ...
          if ((item as PerformanceItem).timer_start_time && ((perf.durationSeconds === undefined || perf.durationSeconds === null) && perf.duration)) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/durable/createItem') {
      const newItemData = await request.json() as { name: string; state: string; type?: ItemType; durationSeconds?: number | null; description?: string | null; style?: string | null; teamSize?: number | null; choreographers?: string | null; duration?: string | null };
      ...
      if (itemType === 'BREAK') {
        ...
        const allowed: PerformanceState[] = ['NONE', 'CHECKED IN', 'BACKSTAGE', 'READY TO GO', 'PERFORMING', 'DONE'];
        initialState = allowed.includes(newItemData.state as PerformanceState) ? (newItemData.state as PerformanceState) : 'NONE';
      }
      ...
      if (itemType === 'BREAK') {
        ...
        const computedDurationSeconds = newItemData.durationSeconds ?? parseDurationToSeconds(newItemData.duration ?? null) ?? null;
        newItem = {
          itemId,
          name: newItemData.name,
          type: 'PERFORMANCE',
    ...
    if (request.method === 'POST' && url.pathname === '/api/durable/reorderItems') {
      try {
        ...
        // persist order ids
        await this.stateObj.storage.put('order', this.event.items.map(i => i.itemId));
